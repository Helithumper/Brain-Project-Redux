...
void move(){
  ...
  /*for(int i = 9; i < 12; i++){
    if(colors[i] != 0){hasEnd = true; endColor = colors[i];}
  }
  if(hasEnd){ //If there is an end, write it to the RGB LED and reset the hasEnd variable
    digitalWrite(rgbRed,LOW);
    digitalWrite(rgbGreen,LOW);
    digitalWrite(rgbBlue,LOW);
    if(endColor==1){digitalWrite(rgbRed,HIGH);}
    if(endColor==2){digitalWrite(rgbGreen,HIGH);}
    if(endColor==3){digitalWrite(rgbBlue,HIGH);}
    hasEnd=false;
  }*/
  if(colors[11]!=0){
    digitalWrite(rgbRed,LOW);
